// Kristian Risager Larsen, IoT Lab, Aarhus Kommune, 2026-04
function decode(payload, metadata) {
    ...
    // Add properties in NGSI-LD format
    for (let key in decoded) {
    if (key !== "devicetype" && key !== "battery") {
            getProperty(decoded, payload, key, res);
        }
    }
